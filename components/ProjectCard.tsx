...
const ProjectCard = ({ project }: { project: Project }) => {
  return (
    <section className="group flex gap-x-9 gap-y-4 rounded-lg bg-slate-900/50 p-6 pl-3 duration-300 max-md:flex-col max-md:gap-y-3">
      ...
      <section className='flex flex-col gap-y-4 max-md:flex-row max-md:items-center max-md:justify-between max-md:gap-y-2.5 max-sm:flex-col'>
        <h1 className="text-wrap text-sm font-medium text-slate-100 duration-300 group-hover:text-corral max-md:text-base max-sm:text-center max-sm:text-sm">
          {project.title}
        </h1>
        ...
        <h2 className="text-sm font-light text-slate-300 max-md:text-center max-sm:text-xs">
          {project.description}
        </h2>
